  ...
  try {
    ...
    if (tagsToRemove.length > 0) {
      ...
      await TagQuestion.deleteMany(
        { tag: { $in: tagsIdsToRemove }, question: questionId },
        { session }
      );
...
): Promise<ActionResponse<{ questions: QuestionTypes[]; isNext: boolean }>> {
  const validationResult = await action({
    params,
...
export async function deleteQuestion(params: DeleteQuestionParams): Promise<ActionResponse<void>> {
  const validationResult = await action({
    params,
